    ...
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          {/* IMPORTANT: Add Lead must NOT go to /leads/new (dynamic [id] catches "new") */}
          <Link
            href="/leads?new=1"
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-sm text-zinc-600">
              Jo follow-ups due hain, unko pehle clear karo — pipeline fast move
              hogi.
            </div>
          </div>
          ...
          <Link
            ...
            className="rounded-xl bg-black px-4 py-2 text-sm font-medium text-white hover:opacity-90"
          >
            Open Follow-ups ({kpis.followups_due})
          </Link>
        </div>

        <div className="mt-3 rounded-xl border border-zinc-200 bg-zinc-50 p-3 text-sm text-zinc-700">
          You have <span className="font-semibold">{kpis.followups_due}</span>{" "}
          follow-up(s) due. Open the list and clear them.
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-5">
          {(["New", "Contacted", "Follow-Up", "Booked", "Lost"] as const).map(
            (s) => (
              <div
                key={s}
